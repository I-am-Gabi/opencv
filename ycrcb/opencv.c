...
int toYcrcbDirect(IplImage* img) {
	...
	// IplImage* cvCreateImage(CvSize size, int depth, int channels);
	IplImage *imgY = cvCreateImage(cvSize(img->width,img->height), img->depth, img->nChannels);
	  
	// cvConvertImage(src, dst, flags=0);
	cvCvtColor(img, imgY, CV_RGB2YCrCb); //Change from img to YCR 
	 
	cvNamedWindow("YCR:", 1);
	cvNamedWindow("YCRCB", CV_WINDOW_AUTOSIZE);

	cvShowImage("YCR:", imgY); 
